...
async function handleCheckoutCompleted(session: Stripe.Checkout.Session) {
  ...
  try {
    ...
    // Get line items from Stripe, expanded so we can match by productId metadata
    const lineItemsResponse = await stripe.checkout.sessions.listLineItems(
      session.id,
      { expand: ["data.price.product"] }
    );

    // Map our productId → unit price (line items may not be in metadata order)
    ...
    for (const item of lineItemsResponse.data) {
      ...
      if (
        ...
        const productId = (stripeProduct as Stripe.Product).metadata?.productId;
        if (productId) {
          priceByProductId.set(productId, (item.price?.unit_amount ?? 0) / 100);
        }
      }
    }
