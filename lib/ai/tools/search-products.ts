...
import type { AI_SEARCH_PRODUCTS_QUERYResult } from "@/sanity.query-types";
...
const productSearchSchema = z.object({
  query: z
    ...
    .default("")
    .describe(
      "Search term to find products by name, description, or category (e.g., 'pearl necklace', 'stud earrings', 'bridal')"
    ),
  category: z
    .string()
    .optional()
    .default("")
    .describe(
      "Filter by category slug (e.g., 'quality-necklaces', 'quality-earrings', 'hair-accessories', 'bridal-accessories')"
    ),
  material: z
  ...
  minPrice: z
    ...
    .optional()
    .default(0)
    .describe("Minimum price in KSh (e.g., 1000)"),
  maxPrice: z
    .number()
    .optional()
    .default(0)
    .describe("Maximum price in KSh (e.g., 5000). Use 0 for no maximum."),
});

export const searchProductsTool = tool({
  description:
    "Search for products in the jewelry and accessories store. Can search by name, description, or category, and filter by material, color, and price range. Returns product details including stock availability.",
