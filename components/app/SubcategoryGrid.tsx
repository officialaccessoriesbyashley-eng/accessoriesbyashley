...
interface SubcategoryGridProps {
  ...
  /** Used to build deselect href when baseHref is not supplied */
  categorySlug?: string;
  activeSub?: string;
        ...
        {subcategories.map((sub) => {
          ...
          return (
            <Link
              key={sub._id}
              href={href}
              className={cn("group block transition-opacity duration-200", hasActive && !isActive && "opacity-40")}
            >
              {/* Square image */}
              <div
                className={cn(
              ...
              <div className="mt-3 px-1">
                ...
                >
                  {sub.title}
                  {isActive && (
                    <span className="ml-2 text-xs font-normal text-zinc-500 dark:text-zinc-400">(tap to clear)</span>
                  )}
                </p>
              </div>
            </Link>
